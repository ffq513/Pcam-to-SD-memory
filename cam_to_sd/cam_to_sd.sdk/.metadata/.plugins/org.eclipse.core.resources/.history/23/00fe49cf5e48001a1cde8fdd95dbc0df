...
//		while (XAxiVdma_ChannelIsRunning(&drv_inst_.ReadChannel)) ;
	...
	void copyDataFrame(u8* buffer_data, int pxl_size)
	{
		...
		{
			buffer_data[buffer_pos] = Xil_In8(frame_base_addr);
			buffer_data[buffer_pos+1] = Xil_In8(frame_base_addr+1);
			buffer_data[buffer_pos+2] = Xil_In8(frame_base_addr+2);
		}
		XAxiVdma_ChannelStart(&(drv_inst_.WriteChannel));
	}
...
//		while (XAxiVdma_ChannelIsRunning(&drv_inst_.WriteChannel));
	...
	void configureWrite(uint16_t h_res, uint16_t v_res)
	{
		...
		context_.hsize = h_res;
		context_.WriteCfg.VertSizeInput = v_res;
		context_.vsize = v_res;
		...
		for (int iFrm=0; iFrm<drv_inst_.MaxNumFrames; ++iFrm) {
			...
			xil_printf("VDMA Frame %d Addr to write: 0x%08x\r\n", iFrm, addr);
